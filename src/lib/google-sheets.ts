...
export class GoogleSheetsClient {
  private static auth = new google.auth.GoogleAuth({
    keyFile: path.join(process.cwd(), process.env.GOOGLE_APPLICATION_CREDENTIALS || 'kunci_rahasia_google.json'),
    scopes: ['https://www.googleapis.com/auth/spreadsheets.readonly'],
  });

  private static sheets = google.sheets({ version: 'v4', auth: this.auth });

  static async getSheetData(range: string): Promise<any[][]> {
  ...
  static async getRowsFromGid(gid: string, rangeOffset: string = 'A1:ZZ'): Promise<any[][]> {
